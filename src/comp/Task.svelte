  ...
  <div id="tasktype_choice">
    <div class="relative-wrap">
      <div id="kebab">
        ...
        <div id="kebab-popup">
          <svg on:click={() => (task.type = TaskType.Event)} viewBox="0 0 16 16">
            <path fill="currentColor" d={getTaskTypeSVGPath(TaskType.Event)} />
          </svg>
          <svg on:click={() => (task.type = TaskType.Task)} viewBox="0 0 16 16">
            <path fill="currentColor" d={getTaskTypeSVGPath(TaskType.Task)} />
          </svg>
          <svg on:click={() => (task.type = TaskType.Note)} viewBox="0 0 16 16">
    ...
    <div id="chosen-type-wrap" class="relative-wrap">
      ...
      <input id="done-check" type="checkbox" bind:checked={task.done} />
      {:else}
      <svg class="svg_80" class:no-value={!task} viewBox="0 0 16 16">
        <path fill="currentColor" d={getTaskTypeSVGPath(task.type)} />
      </svg>
      {/if}
    </div>
  </div>
  <input placeholder="Ievadiet uzdevuma/notikuma/piezīmju tekstu" class="task-text-input" class:strikeout={task.done} type="text" bind:value={task.text} />
  {#if task.ref}
